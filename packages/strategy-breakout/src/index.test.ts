...
describe('breakout strategy', () => {
  ...
  it('allows an exit even when the entry capital ceiling is already consumed', () => expect(analyzeBreakout({ ...parameters, maxCapital: 10 }, { now: 100_000, candles: candles(96), spentCapital: 10, lastTradeAt: null, positionQuantity: 0.1, averageEntryPrice: 100, mode: 'PAPER' })).toMatchObject({ proposals: [{ side: 'SELL', quantity: 0.1 }] }));
});

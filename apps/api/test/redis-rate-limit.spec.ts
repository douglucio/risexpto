...
describe('Redis rate limiter', () => {
  it('passes through when Redis is not configured', async () => {
    ...
    await limiter.middleware(
      { method: 'GET', path: '/profile', headers: {} },
      { setHeader: vi.fn(), status: vi.fn() as never },
      next,
    );
  ...
  it.skipIf(!process.env.E2E_REDIS_URL)(
    'returns 429 after the configured Redis window quota',
    async () => {
      ...
      try {
        for (let index = 0; index < 3; index += 1)
          await limiter.middleware(
            { method: 'GET', path, ip: 'rate-test', headers: {} },
            response,
            next,
          );

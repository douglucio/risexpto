...
export class QueueService implements OnModuleDestroy {
  ...
  constructor() {
    ...
    this.queue = redisUrl
      ? new Queue<CycleJob>(process.env.WORKER_QUEUE_NAME ?? 'risexpto', {
          connection: redisConnection(redisUrl) as never,
          prefix: 'risexpto',
        })
      : null;
  }
  async enqueueBotCycle(botId: string, idempotencyKey: string) {
    ...
    if (await this.queue.getJob(idempotencyKey))
      throw new ConflictException('Cycle already queued');
    return this.queue.add(
      'bot-cycle',
      { type: 'bot-cycle', botId },
      {

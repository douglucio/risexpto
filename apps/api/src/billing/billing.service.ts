...
export class BillingService {
      ...
      subscription: subscription
        ? {
            ...
            entitlements: Object.fromEntries(
              subscription.plan.entitlements.map((item) => [item.key, item.value]),
            ),
          }
        : null,
    };
  ...
  async checkout(user: AuthenticatedUser, body: Record<string, unknown>) {
    ...
    const providerCustomerId =
      customer?.providerCustomerId ?? (await this.stripe().createCustomer(userId, user.email));
  ...
  async portal(user: AuthenticatedUser) {
    const customer = await this.db.billingCustomer.findUnique({
      where: { userId: applicationUserId(user) },
    });
  ...
  async webhook(rawBody: Buffer, signature: string) {
    ...
    const existing = await this.db.billingWebhookEvent.findUnique({
  ...
  private async applySubscriptionEvent(db: Prisma.TransactionClient, event: Stripe.Event) {
    ...
    const customer = await db.billingCustomer.findUnique({
      where: { providerCustomerId: object.customer },
    });
    if (!customer) return;
    ...
    await db.subscription.upsert({
      ...
      create: {
        ...
        status,
        currentPeriodStart: dateFromSeconds(object.start_date),
        currentPeriodEnd: dateFromSeconds(periodEnd),
...
function applicationUserId(user: AuthenticatedUser): string {
  if (!user.applicationUserId) throw new ServiceUnavailableException('User is not provisioned');
...
function mapStatus(
  ...
  if (
    ...
    return status.toUpperCase() as ReturnType<typeof mapStatus>;
  return 'INCOMPLETE';
}

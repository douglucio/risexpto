...
export function PublicPricing() {
  ...
  useEffect(() => {
    void fetch('/api/public/plans')
      .then(async (response) =>
        response.ok ? setPlans((await response.json()) as Plan[]) : setError(true),
      )
      .catch(() => setError(true));
  }, []);
  ...
  return (
    <div className="pricing-grid">
      {plans.map((plan) => (
        <article
          key={plan.plan}
          className={plan.recommended ? 'pricing-card is-recommended' : 'pricing-card'}
        >
          <div>
            <h3>{plan.displayName}</h3>
            <strong>{formatCurrency(plan.monthlyPrice, plan.currency, locale)}</strong>

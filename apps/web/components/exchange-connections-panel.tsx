...
export function ExchangeConnectionsPanel({ initial }: { initial: Connection[] }) {
  ...
  async function revoke(id: string) {
    ...
    try {
      ...
      setMessage(error instanceof Error ? error.message : 'Could not revoke connection.');
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <div className="content-stack">
      <Card className="content-stack">
        ...
        <p>Binance is available now. Other providers are shown as roadmap options only.</p>
      ...
      <Card className="content-stack">
        ...
        <p>
          Use a Spot Testnet key with withdrawals disabled. The API secret is encrypted and never
          returned.
        </p>
        ...
        <Button
          ...
          onClick={() => void add()}
        >
          Save Testnet connection
        </Button>
      </Card>
      {message ? (
        <Alert tone="info" title="Exchange connection">
          {message}
        </Alert>
      ) : null}

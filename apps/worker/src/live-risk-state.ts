...
export async function refreshLiveRiskState(database: PrismaClient, botId: string): Promise<void> {
  ...
  const open = positions.filter(
    (position) => position.status === 'OPEN' && new Decimal(position.quantity).greaterThan(0),
  );
  ...
  for (const position of open) {
    ...
    if (!snapshot || new Decimal(snapshot.close).lte(0))
      throw new Error(`LIVE_RISK_PRICE_UNAVAILABLE:${position.symbol}`);
    prices.set(position.symbol, new Decimal(snapshot.close));
  }
  const exposure = open.reduce(
    (total, position) =>
      total.plus(new Decimal(position.quantity).times(prices.get(position.symbol)!)),
    new Decimal(0),
  );
  const realizedPnl = positions.reduce(
    (total, position) => total.plus(new Decimal(position.realizedPnl)),
    new Decimal(0),
  );
    ...
    (total, position) =>
      total.plus(
        new Decimal(position.quantity).times(
          prices.get(position.symbol)!.minus(position.averagePrice),
        ),
      ),
    new Decimal(0),
  );
  const equity = new Decimal(bot.configuration.authorizedCapital)
    .plus(realizedPnl)
    .plus(unrealizedPnl);
  const previous = await database.liveRiskState.findUnique({
    where: { botId },
    select: { id: true, peakEquity: true },
  });
  ...
  const dailyRealized = todayTrades.reduce(
    (total, trade) => total.plus(new Decimal(trade.realizedPnl)),
    new Decimal(0),
  );
  const dailyLoss = Decimal.max(new Decimal(0), dailyRealized.negated());
  const peakEquity = Decimal.max(
    new Decimal(previous?.peakEquity ?? bot.configuration.authorizedCapital),
    equity,
  );
  const drawdown = peakEquity.greaterThan(0)
    ? Decimal.max(new Decimal(0), peakEquity.minus(equity).dividedBy(peakEquity))
    : new Decimal(1);

  ...
  return [
    {
      ...
      detail:
        env.BINANCE_TRADING_ENVIRONMENT === 'TESTNET'
          ? 'TESTNET'
          : 'requires BINANCE_TRADING_ENVIRONMENT=TESTNET',
    },
    {
      name: 'endpoint',
    ...
    },
    {
      ...
      detail:
        env.LIVE_TRADING_ENABLED === 'true'
          ? 'LIVE enabled only on TESTNET'
          : 'LIVE disabled by default',
    },
    {
      name: 'api-key',
      ok: Boolean(env.BINANCE_TESTNET_API_KEY),
      detail: env.BINANCE_TESTNET_API_KEY
        ? 'configured (value hidden)'
        : 'missing BINANCE_TESTNET_API_KEY',
    },
    {
      name: 'api-secret',
      ok: Boolean(env.BINANCE_TESTNET_API_SECRET),
      detail: env.BINANCE_TESTNET_API_SECRET
        ? 'configured (value hidden)'
        : 'missing BINANCE_TESTNET_API_SECRET',
    },
    {
      name: 'database',
      ...
      detail:
        env.DATABASE_URL || env.E2E_DATABASE_URL
          ...
          : 'missing DATABASE_URL/E2E_DATABASE_URL',
    },
    {
      name: 'redis',
      ok: Boolean(env.REDIS_URL || env.E2E_REDIS_URL),
      detail:
        env.REDIS_URL || env.E2E_REDIS_URL
          ? 'connection string configured'
          : 'missing REDIS_URL/E2E_REDIS_URL',
    },
  ];
}
...
if (process.env.NODE_ENV !== 'test') {
  ...
  console.log(
    JSON.stringify({ environment: 'TESTNET', ready: checks.every((check) => check.ok), checks }),
  );

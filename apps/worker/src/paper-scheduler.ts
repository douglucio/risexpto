...
export async function schedulePaperCycles(
  ...
  for (const bot of bots) {
    ...
    const nextRunAt = new Date(Math.max(now.getTime() + intervalMs, scheduledAt.getTime() + intervalMs));
    const claimed = await database.bot.updateMany({
      where: { id: bot.id, status: 'RUNNING', tradingMode: 'PAPER', archivedAt: null, nextRunAt: bot.nextRunAt },
      data: { nextRunAt },
    });
    if (claimed.count !== 1) continue;
    await queue.add('bot-cycle', { type: 'bot-cycle', botId: bot.id }, {
      jobId: paperCycleJobId(bot.id, scheduledAt.getTime()), attempts: 3,
      backoff: { type: 'exponential', delay: 1_000 }, removeOnComplete: { age: 86_400, count: 10_000 }, removeOnFail: false,
    });
    scheduled += 1;
  }
  return scheduled;
...
function intervalFrom(parameters: unknown): number | null {
  ...
  return typeof value === 'number' && Number.isInteger(value) && value >= 1_000 && value <= 86_400_000 ? value : null;
}

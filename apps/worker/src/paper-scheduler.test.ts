...
describe('paper scheduler', () => {
  it('claims due PAPER bots and enqueues deterministic cycles', async () => {
    ...
    const database = {
      bot: { findMany: vi.fn().mockResolvedValue([
        ...
        { id: 'bot-2', nextRunAt: new Date(now.getTime() + 1_000), configuration: { parameters: { intervalMs: 60_000 } } },
    ...
    const claim = updateMany.mock.calls[0] as unknown as [{ where: { id: string; nextRunAt: Date | null } }];
    expect(claim[0].where).toMatchObject({ id: 'bot-1', nextRunAt: null });
    const call = add.mock.calls[0] as unknown as [string, { type: string; botId: string }, { jobId: string }];
  ...
  it('does not enqueue when another scheduler wins the atomic claim', async () => {
    const database = {
      bot: { findMany: vi.fn().mockResolvedValue([{ id: 'bot-1', nextRunAt: null, configuration: { parameters: { intervalMs: 60_000 } } }]), updateMany: vi.fn().mockResolvedValue({ count: 0 }) },
    } as unknown as PrismaClient;
    const add = vi.fn();

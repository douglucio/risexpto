...
export class PortfolioRiskStateService {
  ...
  async load(botId: string) {
    const bot = await this.database.bot.findUniqueOrThrow({ where: { id: botId }, select: { userId: true, exchangeConnectionId: true, paperPortfolioId: true } });
    ...
    return { allocatedCapital: allocated, currentExposure: exposure, dailyLoss: Decimal.max(0, new Decimal(realized._sum.realizedPnl ?? 0).negated()), activeTraders: bots.length };
  }
}

...
async function main(): Promise<void> {
  ...
  console.info(
    JSON.stringify({
      event: 'binance_testnet_readiness_ok',
      symbol: rules.symbol,
      status: rules.status,
  ...
  if (process.env.BINANCE_TESTNET_SMOKE_ORDER !== 'true') {
    console.info(
      JSON.stringify({
        event: 'binance_testnet_order_skipped',
  ...
  console.info(
    JSON.stringify({
      event: 'binance_testnet_order_submitted',
  ...
  if (!['FILLED', 'CANCELED', 'REJECTED'].includes(observed.status)) {
    ...
    console.info(
      JSON.stringify({
        event: 'binance_testnet_order_canceled',
        externalOrderId: canceled.externalOrderId,
  ...
  console.info(
    JSON.stringify({
      event: 'binance_testnet_order_reconciled',
      externalOrderId: final.externalOrderId,
      status: final.status,
...
if (process.env.NODE_ENV !== 'test')
  void main().catch((error: unknown) => {
    console.error(
      JSON.stringify({
        event: 'binance_testnet_smoke_failed',

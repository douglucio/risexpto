  ...
  it('approves only one concurrent reservation within the global limit', async () => {
    ...
    const database = {
      paperCapitalReservation: {
        ...
        create: vi.fn(({ data }: { data: { proposalId: string } }) => {
          reservations.add(data.proposalId);
        }),
      },
      paperCapitalAllocation: {
        upsert: vi.fn(({ where }: { where: { botId: string } }) => {
          if (!botAllocations.has(where.botId)) botAllocations.set(where.botId, 0);
        }),
        updateMany: vi.fn(
          ({
            where,
            ...
            where: { botId: string; allocated: { lte?: number; gte?: number } };
            data: { allocated: { increment?: number; decrement?: number } };
          }) => {
            const current = botAllocations.get(where.botId) ?? 0;
            if (where.allocated.lte !== undefined && current > where.allocated.lte)
              return { count: 0 };
            if (where.allocated.gte !== undefined && current < where.allocated.gte)
              return { count: 0 };
            botAllocations.set(
              where.botId,
              current + (data.allocated.increment ?? 0) - (data.allocated.decrement ?? 0),
            );
            return { count: 1 };
          },
        ),
      ...
      botConfiguration: {
        aggregate: vi.fn().mockResolvedValue({ _sum: { authorizedCapital: 100 } }),
      },
      paperGlobalCapitalAllocation: {
        ...
        updateMany: vi.fn(
          ({
            ...
            where: { id: string; allocated: { lte: number } };
            data: { allocated: { increment: number } };
          }) => {

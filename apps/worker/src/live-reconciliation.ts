...
export async function reconcileLiveOrders(database: PrismaClient, exchangeConnectionId: string): Promise<number> {
  ...
  const contexts = new Map(orders.map((order) => [order.clientOrderId, {
    botId: order.botId,
    exchangeConnectionId,
    tradeProposalId: order.proposal.id,
    idempotencyKey: `live:${order.id}`,
  }]));
  const { engine, connector } = await createTestnetLiveExecutionEngine(database, exchangeConnectionId, (request) => {
  ...
  for (const order of orders) {
    ...
    if (blocked) {
      await recordEvent(database, order.botId, order.proposal.correlationId, 'LIVE_RECONCILIATION_BLOCKED', {
        orderId: order.id, scope: blocked.scope, reason: blocked.reason,
      });
      continue;
    }
    try {
      ...
      await recordAudit(database, order.botId, order.proposal.correlationId, 'ORDER_RESULT', {
        orderId: order.id, clientOrderId: order.clientOrderId, action: 'RECONCILE',
      });
    } catch (error) {
      await recordEvent(database, order.botId, order.proposal.correlationId, 'LIVE_RECONCILIATION_FAILED', {
        orderId: order.id, clientOrderId: order.clientOrderId, error: safeError(error),
      });
    }
  }
  return reconciled;
...
async function recordEvent(database: PrismaClient, botId: string, correlationId: string, type: string, payload: object) {

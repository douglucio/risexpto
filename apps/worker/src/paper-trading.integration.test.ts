...
describe('paper trading persistence integration', () => {
  it.skipIf(!databaseUrl)(
    'persists a complete PAPER cycle in PostgreSQL',
    async () => {
      ...
      try {
        ...
        await database.bot.create({
          data: {
            ...
            configuration: {
              create: {
                parameters: { symbol: 'BTCUSDT', intervalMs: 1, quoteAmount: 10, maxCapital: 100 },
        ...
        await processPaperCycle(database, {
          id: `e2e-${suffix}`,
          data: { type: 'bot-cycle', botId },
        } as never);
        await processPaperCycle(database, {
          id: `e2e-${suffix}`,
          data: { type: 'bot-cycle', botId },
        ...
        const [proposal, riskEvents, order, trades, position, balance, botEvents] =
          await Promise.all([

...
describe('persistent worker queue integration', () => {
  it.skipIf(!redisUrl)(
    'persists and processes a PAPER cycle job through Redis',
    async () => {
      ...
      try {
        const completion = new Promise<void>((resolve, reject) => {
          const timeout = setTimeout(
            () => reject(new Error('Timed out waiting for worker job')),
            10_000,
          );
        ...
        const job = await infrastructure.queue.add(
          'bot-cycle',
          { type: 'bot-cycle', botId: 'e2e-bot' },
          { jobId: 'e2e-cycle' },
        );
        await completion;
  ...
  it.skipIf(!redisUrl)(
    'retries a failed job without losing it',
    async () => {
      ...
      try {
        const completion = new Promise<void>((resolve, reject) => {
          const timeout = setTimeout(
            () => reject(new Error('Timed out waiting for retry')),
            10_000,
          );
          infrastructure.events.on('completed', ({ jobId }) => {
        ...
        await infrastructure.queue.add(
          'bot-cycle',
          { type: 'bot-cycle', botId: 'e2e-bot' },
          {
            jobId: 'e2e-retry',

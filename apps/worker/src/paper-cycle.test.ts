...
describe('processPaperCycle', () => {
  it('persists a safe completion when market data is unavailable', async () => {
    ...
    expect(create.mock.calls[1]?.[0]).toEqual({
      data: {
        ...
        payload: { jobId: 'job-1', reason: 'MARKET_DATA_UNAVAILABLE' },
      },
    });
  });
  ...
  it('blocks a cycle when a persistent kill switch is active', async () => {
    ...
    const database = {
      ...
      killSwitchState: {
        findFirst: vi.fn().mockResolvedValue({ scope: 'SYSTEM', reason: 'incident' }),
      },
      botEvent: { create },
    } as never;
    await processPaperCycle(database, {
    ...
    expect(create).toHaveBeenCalledWith({
      data: {
        botId: 'bot-1',
        type: 'CYCLE_BLOCKED',
        payload: { jobId: 'job-2', scope: 'SYSTEM', reason: 'incident' },
      },
    });
  });
});

    ...
    await database.$transaction([
      ...
      database.botEvent.create({
        data: {
          ...
          payload: {
            orderId: order.id,
            expectedFilledQuantity: expected.toString(),
            observedFilledQuantity: observed.toString(),
          },
        },
      }),
    ]);
...
export async function recoverOrphanedReservations(
  ...
  for (const reservation of reservations) {
    ...
    await database.$transaction([
      database.paperCapitalReservation.updateMany({
      ...
      database.botEvent.create({
        data: {
          ...
          type: 'CAPITAL_RESERVATION_RELEASED',
          payload: { reservationId: reservation.id, reason: 'ORPHANED' },
        },
      }),
    ]);
  }

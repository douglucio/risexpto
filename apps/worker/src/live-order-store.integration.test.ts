...
describe('LIVE order persistence integration', () => {
  it.skipIf(!databaseUrl)(
    'recovers a pending order after a duplicate worker and restart',
    async () => {
      ...
      try {
        await database.user.create({
          data: { id: userId, externalAuthId: `e2e-${suffix}`, email: `${suffix}@example.com` },
        });
        await database.exchangeConnection.create({
          data: {
            ...
            apiSecretCiphertext: Buffer.from('secret'),
            encryptionKeyVersion: 1,
          },
        });
        await database.strategyDefinition.create({
          data: {
            id: definitionId,
            key: `e2e-${suffix}`,
            name: 'E2E LIVE',
        ...
        ).resolves.toMatchObject({ state: 'PENDING_SUBMIT' });

        const result = {
          clientOrderId,
          externalOrderId: `exchange-${suffix}`,
        ...
        await expect(
          new LiveExecutionEngine(connector, true, firstStore).submit(request),
        ).resolves.toEqual(result);
        await expect(
          new LiveExecutionEngine(
            { submit: vi.fn(), query: vi.fn(), cancel: vi.fn() },
            true,
            restartedStore,
          ).submit(request),
        ).resolves.toEqual(result);

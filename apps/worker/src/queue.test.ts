...
describe('LIVE queue idempotency', () => {
  ...
  it('enqueues duplicate-safe LIVE jobs with the same id', async () => {
    ...
    expect(add).toHaveBeenNthCalledWith(
      1,
      'live-submit',
      { type: 'live-submit', orderId: 'order-1' },
      { jobId: 'live-submit:order-1' },
    );
    expect(add).toHaveBeenNthCalledWith(
      2,
      'live-submit',
    ...
    expect(add).toHaveBeenNthCalledWith(
      3,
      'live-reconcile',
      { type: 'live-reconcile', exchangeConnectionId: 'connection-1' },
      { jobId: 'live-reconcile:connection-1' },
    );
  });
});

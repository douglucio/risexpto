...
if (!email || !email.includes('@')) throw new Error('Usage: pnpm dev:grant-plan <email> [STARTER|PRO]');
if (!['STARTER', 'PRO'].includes(planKey)) throw new Error('Development plan must be STARTER or PRO');
...
try {
  ...
  const plan = await db.plan.findUnique({ where: { key: planKey }, select: { id: true, key: true } });
  ...
  console.log(JSON.stringify({ email: user.email, plan: subscription.plan.key, status: subscription.status, source: 'development-local-grant', stripe: false }));
} finally {
  await db.$disconnect();
}
